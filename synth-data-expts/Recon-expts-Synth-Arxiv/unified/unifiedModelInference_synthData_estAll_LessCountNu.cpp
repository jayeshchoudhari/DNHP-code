...
int main()
{
	...
	allEvents = getEventsFromFile("../centralFiles/events_1L.txt");
	// allEvents = getEventsFromFile("./centralFiles/events_10.txt");
	cout << "Got all the events --- " << allEvents.size() << endl;
	
	// Read all the docs...
	allEventsDocs = getSyntheticDocsFromFile("../centralFiles/docs_1L.txt");
...
vector< vector <int> > initializeEvents(vector< vector <int> > allEvents, int flag)
{
	...
	for(ui i = 0; i < allEvents.size(); i++)
	{
		...
		{
			possParentEventId = allPossibleParentEvents[i][0];
...
int sampleUserUserInfluence(int ITE)
{
	...
	for(ui i = 0; i < userGraphMap.size(); i++)
	{
		...
		for(int topicId = 0; topicId < numTopics; topicId++)
		{
			totalNu += (nodeTopicCountMap[uNode][topicId] * topicTopicInfluenceSumPerTopic[topicId]);
		}
...
int sampleTopicTopicInfluence(int ITE)
{
	...
	for(int i = 0; i < numTopics; i++)
	{
		...
		for(int j = 0; j < numTopics; j++)
		{
			...
			// cout << kTopic << " " << kPrimeTopic << " " << TkkCount << " " << totalTk <<  " " << tkk << endl;
			// tkkFile << kTopic << " " << kPrimeTopic << " " << topicTopicCount[kTopic][kPrimeTopic] << " " << totalTk << " " << tkk << endl;
		}
...
int sampleTopicAssignment(int ITE)
{
	...
	cout << "\nLog Likelihood After topic Assignment = " << logLikelihood << "\n";
...
int sampleParentAssignment(int ITE)
{
	...
	for(unsigned int i = 0; i < newSyntheticEvents.size(); i++)
	{
		...
		int assignedParent = getSampledParentAssignment(eventTime, eventNode, eventIndex, eventTopic, ITE);
...
int getSampledParentAssignment(double eventTime, li eventNode, li eventIndex, li eventTopic, int ITE)
{
	...
	if(possibleParentEvents.size() > 0)
	{
		...
		// for(int p = 0; p < calculatedProbVec.size(); p++)
		// {
		// 	cout << " " << p << ":" << calculatedProbVec[p];
		// }
		...
		double eventLLVal = log(calculatedProbVec[sampledIndex]);
		logLikelihood += eventLLVal;

		// cout << " Prob:" << calculatedProbVec[sampledIndex] << " poss parents = " << possibleParentEvents.size() << " event log-likelihood : " << eventLLVal << endl;
...
vector<double> populateCalculatedProbVec(vector <ui> possibleParentEvents, vector<double> possibleParentExp, int eventNode, int eventTopic, double eventTime, int ITE)
{
	...
	{
		cout << " sum Prob = " << normalizationFactor;
		for(int p = 0; p < calculatedProbVec.size()-1; p++)
		{
			vector <int> possParentEvent = newSyntheticEvents[possibleParentEvents[p]];
...
int writeTopicTopicInfluenceToFile()
{
	...
	unordered_map <ui, unordered_map<ui, double> >::iterator topicTopicInfluenceIterator;

	for(topicTopicInfluenceIterator = topicTopicInfluence.begin(); topicTopicInfluenceIterator != topicTopicInfluence.end(); topicTopicInfluenceIterator++)
	{
		ui kTopic = topicTopicInfluenceIterator->first;
		unordered_map<ui, double> topicInf = topicTopicInfluenceIterator->second;

		if(topicInf.size() > 0)
		{
			tkkinfFile << topicInf.size() << " " << kTopic << " ";

			unordered_map<ui, double>::iterator topicInfIterator;

			for(topicInfIterator = topicInf.begin(); topicInfIterator != topicInf.end(); topicInfIterator++)
			{
				tkkinfFile << topicInfIterator->first << " " << topicInfIterator->second << " ";
			}
