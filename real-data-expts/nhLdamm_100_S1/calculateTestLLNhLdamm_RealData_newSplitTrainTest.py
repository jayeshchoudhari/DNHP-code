...
def getTopicPopularityDistribution(fileName):

	tPFile = open(fileName)
	totalPop = 0
	popVec = []
	for line in tPFile:
		flds = line.strip().split()
		tid = int(flds[0].strip())
		pop = int(flds[1].strip())
		totalPop += pop
		popVec.append(pop)

	popVecDist = [(x*1.0)/totalPop for x in popVec]
...
print("Getting Topic popularity...")
# TTInt = getTopicPopularityDistribution("topicPopularity.txt")
topicPopularityDist = getTopicPopularityDistribution("topicPopularity.txt")
print("Got Topic popularity distribution...")
		...
		for topicId in range(numTopics):
			...
			for eachParId in eventCandParents:
				...
				if allEvents[eachParId][4] >= 0 and allEvents[eachParId][4] <= maxTrainLevel:
					...
					parProb = topicPopularityDist[eTopic] * np.exp(-WuvTaukk) * WuvTaukk * np.exp(float(delT))
