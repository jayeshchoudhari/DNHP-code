...
Qout = getUserUserInfluence("userTopicInfAvg.txt")
...
for eachEid in range(len(allEvents)):
	...
	# check if the event is a test event or train event  - Test events have maxTrainLevel set to some higher value...
	if lInfo > maxTrainLevel and len(currDoc) > 0:

		totalProb = 0

		# Marginalize over all possible topic assignments to the test event...
		for topicId in range(numTopics):
			...
			# Marginalize over all possible candidate parents from train set...
			for eachParId in eventCandParents:
				
				#  Check if the candidate parent belongs to Test set or Train set...
                #  allEvents[eachParId][4] give the level infor for the event eachParId...
                #  if the levelInfo for the parId is less than the maxTrainLevel, then the parId is in Train Set...
				if allEvents[eachParId][4] >= 0 and allEvents[eachParId][4] <= maxTrainLevel:
					...
					parProb = np.exp(-WuvTaukk) * WuvTaukk * np.exp(float(delT))
