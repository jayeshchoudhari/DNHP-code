...
int initializeUserTopicInfluenceIn();
int initializeUserTopicInfluenceOut();
...
int sampleUserTopicInfluenceIn(int ITE);
int sampleUserTopicInfluenceOut(int ITE);
...
int writeUserTopicInfluenceInToFile();
int writeUserTopicInfluenceOutToFile();
...
unordered_map<ui, unordered_map<ui, ui> > topicCountInNeighborOfV;
...
unordered_map <ui, unordered_map<ui, double> > userTopicInfluenceIn;
unordered_map <ui, unordered_map<ui, double> > userTopicInfInEvery10thIter;
unordered_map <ui, double> userTopicInfluenceInSumPerNode;

unordered_map <ui, unordered_map<ui, double> > userTopicInfluenceOut;
unordered_map <ui, unordered_map<ui, double> > userTopicInfOutEvery10thIter;
unordered_map <ui, double> userTopicInfluenceOutSumPerNode;
...
int main()
{
	...
	allEvents = getEventsFromFile("../../centralFiles/events_scaledTime_EventSelect_0_testThresh_5_set_1.txt");
    ...
    reverseUserGraphMap = readIntVectorMapFromFile("../../centralFiles/uuRevGraph.txt");
    cout << "Got the followers map... " << reverseUserGraphMap.size() << "\n";
	...
	for(int ITE = 0; ITE < ITERATIONS; ITE++)
	{
        ...
        t1 = high_resolution_clock::now();
		sampleUserTopicInfluenceIn(ITE);
		t2 = high_resolution_clock::now();
	
		duration = duration_cast<microseconds>( t2 - t1 ).count();
		cout << "Sampled User-Topic Influence In for iteration -- "<< ITE << "--  Time taken -- " << duration << endl;
        ...
        t1 = high_resolution_clock::now();
		sampleUserTopicInfluenceOut(ITE);
		t2 = high_resolution_clock::now();
	
		duration = duration_cast<microseconds>( t2 - t1 ).count();
		cout << "Sampled User-Topic Influence Out for iteration -- "<< ITE << "--  Time taken -- " << duration << endl;
	...
	writeUserTopicInfluenceInToFile();
	writeUserTopicInfluenceOutToFile();
...
int initializeBaseRateAndInfluence()
{
	...
	initializeUserTopicInfluenceIn();
	initializeUserTopicInfluenceOut();
...
int initializeUserTopicInfluenceIn()
{
	cout << "Initializing user topic influence In\n";
	...
	for(int i = 0; i < numNodes; i++)
	{
		...
		for(int j = 0; j < numTopics; j++)
		{
            ...
            double shapeParam = nodeTopicCountFromWhere[uNode][kTopic] + baseAlphaTopic;

			double infVal = getSampleFromGamma(shapeParam, scaleParam);

			tempNodeInf[uNode] = infVal;

			userTopicInfluenceInSumPerNode[uNode] += infVal;

			avgVal += infVal;
		}

		userTopicInfluenceIn[uNode] = tempNodeInf;
		tempNodeInf.clear();
...
int initializeUserTopicInfluenceOut()
{
	...
	for(int i = 0; i < numNodes; i++)
	{
		...
		for(int j = 0; j < numTopics; j++)
		{
			...
			userTopicInfluenceOutSumPerNode[uNode] += infVal;

			avgVal += infVal;
		}

		userTopicInfluenceOut[uNode] = tempNodeInf;
		tempNodeInf.clear();
...
/////////////////////////////////////////////////////
/////// SAMPLE USER TOPIC INFLUENCE IN///////////////
/////////////////////////////////////////////////////
...
int sampleUserTopicInfluenceIn(int ITE)
{
	...
	userTopicInfluenceInSumPerNode.clear();
	...
	for(int i = 0; i < numNodes; i++)
	{
		...
		for(int j = 0; j < numTopics; j++)
		{
			...
			// float betaVal = 1.0/topicTopicThetaParamPrior[kPrimeTopic][kTopic];
			float betaVal = 1.0/2;
			
            double totalNuk = 0;

            double topicCountInNeighbor = 0;

            vector<ui> neighborsOfV = reverseUserGraphMap[vNode];
            
            for(ui p = 0; p < neighborsOfV.size(); p++)
		    {
			    ui uNode = neighborsOfV[p];
                topicCountInNeighbor += nodeTopicCountMap[uNode][kTopic];
            }

            topicCountInNeighborOfV[vNode][kTopic] = topicCountInNeighbor;

            for(int kPrimeTopic = 0; kPrimeTopic < numTopics; kPrimeTopic++)
            {
                totalNuk += (topicTopicInfluence[kTopic][kPrimeTopic] * userTopicInfluenceOut[vNode][kPrimeTopic]);
            }
			// cout << vNode << " " << kTopic << " " << totalNuk << "\n";

            totalNuk = totalNuk * topicCountInNeighborOfV[vNode][kTopic];

			double scaleParam = 1/(totalNuk + betaVal);

			double shapeParam = nodeTopicCountFromWhere[vNode][kTopic] + baseAlphaTopic;

			double qivk = getSampleFromGamma(shapeParam, scaleParam);
			
			userTopicInfluenceIn[vNode][kTopic] = qivk;

			userTopicInfluenceInSumPerNode[vNode] += qivk;

			if(ITE >= BURN_IN && ITE%10 == 0)
			{
				userTopicInfInEvery10thIter[vNode][kTopic] += qivk;
			}
		...
		// topicTopicCount[kPrimeTopic].clear();
	}

	// topicTopicCount.clear();

	cout << "Avg Qvk value = " << (avgVal * 1.0) / count  << " Over Edges = " << count << endl;
...
int sampleTopicTopicInfluence(int ITE)
{
	...
	for(int i = 0; i < numTopics; i++)
	{
		...
		for(int j = 0; j < numTopics; j++)
		{
            ...
            for(int vNode = 0; vNode < maxNumNodes; vNode++)
            {
                totalTk += (userTopicInfluenceIn[vNode][kTopic] * userTopicInfluenceOut[vNode][kPrimeTopic] * topicCountInNeighborOfV[vNode][kTopic]);
            }
...
//////////////////////////////////////////////////////
/////// SAMPLE USER TOPIC INFLUENCE OUT///////////////
//////////////////////////////////////////////////////
...
int sampleUserTopicInfluenceOut(int ITE)
{
	...
	userTopicInfluenceOutSumPerNode.clear();
	...
	for(int i = 0; i < numNodes; i++)
	{
		...
		for(int j = 0; j < numTopics; j++)
		{
            ...
            // double topicCountInNeighborOfV = 0;

            vector<ui> neighborsOfV = reverseUserGraphMap[vNode];

            for(int kTopic = 0; kTopic < numTopics; kTopic++)
            {
                totalNuk += (topicCountInNeighborOfV[vNode][kTopic] * topicTopicInfluence[kTopic][kPrimeTopic] * userTopicInfluenceIn[vNode][kTopic]);
            }
			...
			double qovkPrime = getSampleFromGamma(shapeParam, scaleParam);
			
			userTopicInfluenceOut[vNode][kPrimeTopic] = qovkPrime;

			userTopicInfluenceOutSumPerNode[vNode] += qovkPrime;

			if(ITE >= BURN_IN && ITE%10 == 0)
			{
				userTopicInfOutEvery10thIter[vNode][kPrimeTopic] += qovkPrime;
			}
		...
		topicCountInNeighborOfV[vNode].clear();
	}

	topicCountInNeighborOfV.clear();
...
double getEventGenerationTerm(int eventParentIndex, int eventParentNode, int eventParentTopic, int eventIndex, int eventNode, int topicId)
{
	...
	// double hazardTerm = userUserInfluence[eventParentNode][eventNode] * topicTopicInfluence[eventParentTopic][topicId];
	double hazardTerm = userTopicInfluenceIn[eventNode][eventParentTopic] * topicTopicInfluence[eventParentTopic][topicId] * userTopicInfluenceOut[eventNode][topicId];
...
vector<double> populateCalculatedProbVec(vector <ui> possibleParentEvents, vector<double> possibleParentExp, int eventNode, int eventTopic, double eventTime, int ITE)
{
	...
	for(k = 0; k < possibleParentEvents.size(); k++)
	{
		...
		{
			double Wuv, Taukk, Qiuk, Qouk;
			// Wuv = userUserInfluence[possParentNode][eventNode];
			Taukk = topicTopicInfluence[possParentEventTopic][eventTopic];
            Qiuk = userTopicInfluenceIn[eventNode][possParentEventTopic];
            Qouk = userTopicInfluenceOut[eventNode][eventTopic];

            hazFinalTerm = 0;
            
			// if(Wuv > 0 && Taukk > 0)
			if(Qiuk > 0 && Taukk > 0 && Qouk > 0)
			{
				double temporalDecay = possibleParentExp[k];
				double uuTTProdTerm = Qiuk * Taukk * Qouk;
...
int writeUserTopicInfluenceInToFile()
{
	...
	uukinfFile.open("userTopicInfIn.txt");
	uukinfFileAvg.open("userTopicInfInAvg.txt");
	...
	for(int i = 0; i < numNodes; i++)
	{
		...
		{
			kTopic = j;
			uukinfFile << kTopic << " " << userTopicInfluenceIn[uNode][kTopic] << " ";
			uukinfFileAvg << kTopic << " " << userTopicInfInEvery10thIter[uNode][kTopic] / 10 << " ";
		}
...
{
	ofstream uukinfFile, uukinfFileAvg;
	uukinfFile.open("userTopicInfOut.txt");
	uukinfFileAvg.open("userTopicInfOutAvg.txt");
	...
	for(int i = 0; i < numNodes; i++)
	{
		...
		{
			kTopic = j;
			uukinfFile << kTopic << " " << userTopicInfluenceOut[uNode][kTopic] << " ";
			uukinfFileAvg << kTopic << " " << userTopicInfOutEvery10thIter[uNode][kTopic] / 10 << " ";
		}
